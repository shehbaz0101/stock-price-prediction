...
async def lifespan(app: FastAPI):
    ...
    if api_key:
        try:
            ...
            log.info("LLM service ready with Grok API  model=%s", GROK_MODEL)
    ...
    log.info("LLM insight service ready on port 8003")
    yield
    if _grok_client:
        await _grok_client.close()


app = FastAPI(title="LLM Insight Service", version="1.0.0", lifespan=lifespan)
...
async def _grok_insight(
    ...
    system = (
        "You are a concise quantitative financial analyst specialising in Indian equities. "
        "Respond ONLY with valid JSON - no prose, no markdown fences.\n"
        'Schema: {"sentiment":"bullish|bearish|neutral","directional_signal":"buy|sell|hold",'
    ...
    user = (
        f"Ticker: {ticker}\n"
        f"Predicted next-close: Rs.{prediction.get('predicted_close', 'N/A')}\n"
    ...
    try:
        ...
        clean = raw.strip()
        if clean.startswith("```"):
            clean = "\n".join(l for l in clean.splitlines() if not l.startswith("```"))
...
def _rule_based(
    ...
    return {
        ...
        "summary":           (
            f"{ticker} predicted at Rs.{pred_close:.2f} with {confidence:.1f}% "
            f"model confidence - {sentiment} stance."
        ),
        "rationale":         (
            f"The gradient-boosting model forecasts Rs.{pred_close:.2f}. "
